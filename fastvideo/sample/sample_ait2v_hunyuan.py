...
from fastvideo.models.hunyuan.inference_ai2v import HunyuanAI2VideoSampler
...
def parse_args():
    ...
    parser.add_argument("--i2v_mode", action="store_true", help="是否使用i2v模式")
    parser.add_argument("--i2v_resolution", type=str, default="540p", choices=["360p", "720p", "540p"], help="i2v分辨率")
    parser.add_argument("--i2v_condition_type", type=str, default="token_replace", choices=["latent_concat", "token_replace"], help="i2v条件类型")
    parser.add_argument("--i2v_stability", action="store_true", help="是否使用i2v稳定性")
    parser.add_argument("--use_lora", action="store_true", help="是否使用lora")
    parser.add_argument("--lora_path", type=str, default=None, help="lora路径")
    parser.add_argument("--lora_scale", type=float, default=0.5, help="lora比例")
...
def main():
    ...
    # 设置Sampler
    sampler = HunyuanAI2VideoSampler.from_pretrained(models_root_path, args)
    # sampler = None
    ...
    for idx, item in enumerate(rank_data_items):
        ...
        try:
            outputs = sampler.predict(
                ...
                i2v_image_path=item.get("first_frame_path"),
